...
public class RequirementPaginationServiceImpl implements IPaginationCommons<RequirementResponseDTO> {
	
	private final EntityManager entityManager;
	
	@Override
	public Page<RequirementResponseDTO> pagination(PaginationModel pagination) {
		try {
			...
			List<RequirementResponseDTO> lista = querySelect.getResultList();

			PageRequest pageable = PageRequest.of(pagination.getPageNumber(), pagination.getRowsPerPage());

			Page<RequirementResponseDTO> page = new PageImpl<RequirementResponseDTO>(lista, pageable, total);
	...
	public StringBuilder getSelect() {
		StringBuilder sql = new StringBuilder("SELECT new com.bussinesdomain.maestros.dto.RequirementResponseDTO(r.idRequirement,r.title,r.description) ");
        return sql;
	}

	@Override
	public StringBuilder getFrom() {
		StringBuilder sql = new StringBuilder(" FROM RequirementEntity r  ");
        return sql;
	}
        ...
        if(!sorts.isEmpty()){
            ...
            for(SortModel sort:sorts){
                if(sort.getColName().equals("idRequirement")){
                    ...
                    sql.append( " idRequirement " + sort.getSort() );
                    flagMore = true;
                }
