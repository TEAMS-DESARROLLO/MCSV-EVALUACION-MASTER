...
public class TopicPaginationServiceImpl implements IPaginationCommons<TopicResponseDTO> {
	
	private final EntityManager entityManager;
	
	@Override
	public Page<TopicResponseDTO> pagination(PaginationModel pagination) {
		try {
			...
			List<TopicResponseDTO> lista = querySelect.getResultList();

			PageRequest pageable = PageRequest.of(pagination.getPageNumber(), pagination.getRowsPerPage());

			Page<TopicResponseDTO> page = new PageImpl<TopicResponseDTO>(lista, pageable, total);
	...
	public StringBuilder getSelect() {
		StringBuilder sql = new StringBuilder("SELECT new com.bussinesdomain.maestros.dto.TopicResponseDTO(r.idTopic,r.title,r.description) ");
        return sql;
	}

	@Override
	public StringBuilder getFrom() {
		StringBuilder sql = new StringBuilder(" FROM TopicEntity r  ");
        return sql;
	}
	...
	public StringBuilder getFilters(List<Filter> filters) {
        ...
        for(Filter filtro:filters){
            if(filtro.getField().equals("idTopic")){
                sql.append(" AND r.idTopic = :idTopic");
            }
        ...
        if(!sorts.isEmpty()){
            ...
            for(SortModel sort:sorts){
                if(sort.getColName().equals("idTopic")){
                    ...
                    sql.append( " idTopic " + sort.getSort() );
                    flagMore = true;
                }
